...
export const FigmaLink = ({ children, href }: PropsWithChildren<{ href?: string }>) => (
  <a href={href || '#'} target="_blank" rel="noreferrer">
    <FigmaIcon /> {children || 'Open in Figma'}
  </a>
);
...
export const FigmaBlock = ({ of }: any) => {
  ...
  if (
    storyObj?.parameters &&
    storyObj?.parameters['design'] &&
    storyObj?.parameters['design'].type === 'figma' &&
    storyObj?.parameters['design'].url
  ) {
    return <FigmaLink href={storyObj?.parameters['design'].url} />;
  } else {
    return null;
  }
};
